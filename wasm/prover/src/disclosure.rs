...
/// ```
pub fn find_ranges_2(sent: &[u8], recv: &[u8]) -> (Vec<Range<usize>>, Vec<Range<usize>>) {
    // Only hide for host = www.kaggle.com
    let (start, end) = find_start_index(sent, "www.kaggle.com".as_bytes(), "\r".as_bytes());
    if (start, end) == (0, 121) {
        return (vec![0..sent.len()], vec![0..recv.len()]);
    }
